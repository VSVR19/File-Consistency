...
#Unique client name for this client which will be sent to the server to be displayed onto the server GUI

'''
Function client_details reads data from clients and it pauses for a 10 second interval.
It then again reads the file
If any diffrence, it is transmitted to server
INPUT: Update made to the client file by user
OUTPUT: Updates transmitted to the serevr and files made consistent
'''
...
def client_details():
    try:
        ...
        rpcgui.entry.insert(tk.END, "Client 1 welcomes you! "+ "\n")
        rpcgui.entry.insert(tk.END, s.connected_clients(client_name) + "\n")
		#The clients name is transmitted to server here.
    
        while True:
            f = open("C:\\Users\\Venkat\\Desktop\\DS Assignment 2\\Client 1\\AS2_File.txt", "r")
            file_data = f.read()
            ...
            while True:
                ...
                f_sleep = open("C:\\Users\\Venkat\\Desktop\\DS Assignment 2\\Client 1\\AS2_File.txt", "r")
...
'''
The main method creates a object rpcgui, for the MyXMLRPCClient class and uses it to start our XMLRPCClient
'''
